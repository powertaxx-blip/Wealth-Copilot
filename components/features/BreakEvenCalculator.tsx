...
/**
 * Direct port of the vanilla-JS `calcBreakEven()` panel. Same two
 * calculators, same math, same result shape — the difference is state
 * lives in React (via useState + a small persistence hook) instead of
 * being read out of the DOM on every keystroke with
 * document.getElementById(...).value, and the render is JSX returned
 * from the component instead of a big template-literal string.
 */
...
type BreakEvenState = {
  ...
  price: number;
  unitCost: number;
  marginPct: number; // stored as 0–100, matches the old <input>'s displayed value
};
...
export function BreakEvenCalculator() {
  ...
  return (
    ...
    >
      <div className="rounded-lg border p-4" style={{ borderColor: "var(--gold)", background: "var(--line-soft)" }}>
        <b>Mentor&apos;s note:</b> Marvin Gaye asked &quot;what&apos;s going on&quot; — your break-even number answers
        that same question for your business. It&apos;s the line between hoping and knowing.
      </div>
