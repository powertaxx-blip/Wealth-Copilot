...
/**
 * Direct port of the vanilla-JS `ficaOnWages()` helper — the payroll-tax
 * twin of calcSETax() above, used by the S-Corp vs. Sole Prop savings
 * calculator on the Filing Status Guide panel. A W-2 wage splits FICA
 * (Social Security + Medicare) between employee and employer; as the
 * S-Corp owner you effectively cover both halves either way, since you
 * own the business paying the "employer" half.
 */
export function ficaOnWages(wage: number): { employee: number; employer: number; total: number } {
  if (wage <= 0) return { employee: 0, employer: 0, total: 0 };
  const ss = Math.min(wage, SS_WAGE_BASE_2025) * 0.124; // 6.2% + 6.2%
  const medicare = wage * 0.029; // 1.45% + 1.45%
